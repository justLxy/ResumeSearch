...
- [5. 检索流程详解](#5-检索流程详解)
  ...
  - [5.3 RRF 融合排序](#53-rrf-融合排序)
    - [5.3.1 匹配分相同时的候选人质量二级排序](#531-匹配分相同时的候选人质量二级排序)
...
| `services/quality.py` | 候选人质量分（学校/奖项/实习/项目/完整度），仅在匹配分相同时作二级排序 |
...
> **⚠️ 分词不对称对 phrase 查询是陷阱**：`match`/`term` 类查询在这套"索引细切、搜索粗切"下工作良好，但 **phrase（短语连续）查询依赖 token 的位置对齐**，一旦某个词的两种切分**首位置不同**，phrase 就会**静默 0 命中**。原理与修复约束见下一节。

#### phrase 短语查询原理与分词对称性（知识点复习）

这一节把"为什么搜 `北京邮电大学 并发长连接` 召不回白睿"背后的 ES 机制讲透，是一组通用知识点，与本项目具体 bug 的排查复盘见 `PROJECT_REVIEW.md` §31。

**① 倒排索引不只存"词在哪些文档"，还存"词在文档的第几个位置"**

很多人以为倒排索引是 `并发 → [文档A, 文档B]`。实际上每个 token 还记录它在每篇文档里的**位置编号（position）**。可用 ES 的 termvectors API 挖出来看，例如白睿 NetPulse 项目切片的 `text.phrase` 字段：
...
**② `match_phrase` 靠"位置逐个 +1 递增连续"实现短语匹配**

`match_phrase "并发长连接"` 会把查询也切成 token 序列 `[并, 发长, 连接]`，然后去倒排索引查每个词的位置列表，验证能否找到一组**连续递增**的位置：

```
查询序列:   并      发长     连接
要求位置:   P      P+1      P+2      ← 必须逐个 +1 连续

文档实际:
  并    → {84}
  发长  → {85}          84 → 85 = 84+1  ✓
  连接  → {75, 86}      86 = 85+1 ✓   (75 前面无 发长@74，作废)
  找到 84,85,86 连续 → 命中
```

算法本质：**从第一个词的每个位置 P 出发，检查第二个词是否在 P+1、第三个词是否在 P+2……全部对上就是一次短语命中；任何一环接不上，这条起点作废，换下一个起点。** 这也是 `match_phrase` 比普通 `match`（只看"词在不在"、不看顺序）更精确的原因。

可用一个词序打乱的查询验证"位置"是二者的唯一区别（同一份文档、同一批词）：

| 查询 | 词序 | `match`（不看位置） | `match_phrase`（看位置） |
|---|---|---|---|
| `并发长连接` | 正确 | 命中 | **命中**（位置连续） |
| `连接长发并` | 打乱 | 命中（词都在） | **0 命中**（位置接不上） |

**③ 位置能不能对上，前提是 token 本身对不对得上——这就要求分词对称**

phrase 检查位置的前提，是查询切出的 token 在文档里**存在**。若两边用不同分词器，切出的 token 不同，第一步就崩了。`并发长连接` 就是典型：

```
建索引 ik_max_word → [并发, 发长, 连接]   （文档倒排索引里存的是 "并发"）
搜索   ik_smart    → [并,   发长, 连接]   （查询要找的是 "并"）
```

查询要找 `并`，但文档索引里根本没有 `并` 这个 token（存的是 `并发`）→ 短语第一步就找不到起点 → **静默 0 命中**（不报错，词肉眼可见却查不到，最难排查）。

**④ 在 ES 里"选字段"就是"选分词器"——修复只需换字段名**

一个字段可以挂多个**子字段**，各配各的分词器，各有独立的倒排索引：

| 字段 | 建索引分词器 | 查询分词器 | 是否对称 | phrase 结果 |
|---|---|---|---|---|
| `text`（主字段） | ik_max_word | ik_smart | ❌ 不对称 | 静默 0 命中 |
| `text.phrase`（子字段） | ik_smart | ik_smart | ✅ 对称 | 正常命中 |
| `skills_text` | ik_smart | ik_smart | ✅ 对称 | 正常命中 |

`multi_match` 的 `fields` 参数写哪个字段名，ES 就查哪份倒排索引。所以修复的代码实质，就是把 `fields` 里的字段名从主字段换成 `.phrase` 子字段——**代码只改了字符串，但字段名在 ES 里等价于"用哪套分词"**：

```python
# resume_search/services/query_builder.py
# 改前（打不对称主字段 → phrase 静默失效）:  ["title^5", "text^4", "skills_text^5"]
# 改后（打分词对称字段 → phrase 正常命中）:
_PHRASE_RECALL_FIELDS = ["title.phrase^5", "text.phrase^4", "skills_text^5"]
```

> `^5`/`^4` 是**打分权重（boost）**，与分词无关：`title.phrase^5` 表示命中标题得分 ×5。这组权重从改前原样搬来，只把 `title`→`title.phrase`、`text`→`text.phrase`；`skills_text` 本身单一 ik_smart、天生对称，不用改。

**⑤ `slop`：放宽位置连续的严格度**

`match_phrase` 可带 `slop` 参数调节"连续"的松紧：

- `slop:0`（默认）：位置必须严格连续 `P, P+1, P+2`
- `slop:1`：允许总共移动 1 步（中间隔一个词、或轻微换序也算命中）
- `slop` 越大越宽松，趋近无穷时退化成"不看位置"（约等于 `match`）

本项目 `_entity_field_query` 的实体短语用 `slop:1`（校名/公司名允许轻微修饰词间隔），而 `_keyword_or_recall_query` 的 `type:phrase` 用默认 `slop:0`（要求精确连续）。

**一句话串起来**：phrase 靠倒排索引存的**位置**实现连续匹配（②）；位置能对上的前提是 token 先对得上，这要求两侧**分词对称**（③）；在 ES 里选字段即选分词器，所以修复就是把查询字段换成分词对称的 `.phrase` 子字段（④）；连续的严格度由 `slop` 调节（⑤）。
...
Step 6: 排序输出
  - 按 final_score 降序
  - 同分时按 best_rank 升序，再按 resume_id 兜底
  - 【质量 tie-break】把 final_score 量化到 4 位小数分桶后，同桶（相关性实质相同）
    内按候选人质量分降序稳定重排（见 5.3.1），跨桶顺序不变
  - 内部最多保留 1000 个候选人
  - 按 offset/limit 返回当前批次，默认首批返回 100 个
```

#### 5.3.1 匹配分相同时的候选人质量二级排序

RRF 分是 `1/(k+rank)` 之和，本质是**离散**的——rank 相同的候选人会分桶到同一分。此前同分一律回落到 `resume_id` 字典序（`M20260001` 只是编号比 `M20260002` 小，与优劣无关），这是可优化点。

**同分到底有多常见（实测）**：在真实查询上按 4 位小数分桶统计"落在 ≥2 人同桶组"的候选人占比：

| 查询类型 | 同桶候选占比 | 最大同桶组 |
|---|---|---|
| keyword 实体（`哈尔滨工业大学`、`腾讯 阿里巴巴`） | **0%** | 1 |
| 单技能（`Python`） | ~8% | 2 |
| 多技能 / 语义（`Python NLP SQL`、长 NL） | **10–20%** | 2–3 |

结论要有分寸：质量 tie-break **不是高频机制，而是尾部整理器**。头部强相关候选几乎都独占一个桶，质量键不动它们；真正受影响的是中后段那些"RRF 离散撞分"的 2–3 人小簇——把簇内的编号顺序换成质量顺序。它不改变"谁上首屏"，只让同分小簇的相对次序更合理。实体/精确查询（boost 拉得很开）则几乎从不触发。

> **浏览模式是另一回事**：空 query / 纯筛选浏览没有相关性可言（`score` 全 0，等价于"全体同分"），此时**全量**按质量分排序，见下文 5.4。

系统引入一个**候选人质量分**（`services/quality.py` 的 `compute_quality_score`）作为二级排序维度。核心原则符合业界 talent-search 最佳实践：**质量分只在匹配相关性实质相同时才起作用，永不覆盖相关性**。

**① 主分分桶——保证质量只在"相关性相同"时生效**

浮点精确相等几乎不触发，直接比 quality 会让质量分侵蚀相关性。做法是把检索主分量化到固定精度（`score_bucket`，4 位小数）再比较，排序键变为：

```
(-score_bucket(主分), -quality_score, best_rank/pre_rank, resume_id)
```

只有主分分桶相等（相关性实质相同）的候选人才按质量重排；相关性有实质差异的绝不会被质量翻转。

**② 质量分组成（∈ [0,1]，加权求和）**

| 维度 | 权重 | 字段来源 | 归一化方式 |
|------|------|----------|-----------|
| 学校层次 | 0.30 | `candidate.all_schools` ∩ `school_tiers.json` | 取最高档序数分：C9=1.0 / 985·海外QS100=0.85 / 211=0.6 / 双一流=0.5 / 其他=0.1 |
| 奖项 | 0.25 | `awards[].level` | 含金量加权求和后除以饱和阈值（约两个一等奖封顶），防刷奖 |
| 实习 | 0.20 | `internships[]` | 0.5×段数 + 0.3×累计时长 + 0.2×描述密度，各带饱和上限 |
| 项目 | 0.15 | `projects[]` | 0.6×段数 + 0.4×描述密度 |
| 完整度 | 0.10 | 关键字段 | 非空关键字段的填充率 |

> 各子分裁剪到 `[0,1]`，加权和后 `quality_score` 也在 `[0,1]`。项目/实习只衡量经历**本身**的丰富与充实度——它们与 query 的**相关性**已由 RRF/rerank 表达，不在静态质量分里重复计分。

**③ 集成位置——两处，各在同量纲阶段内加键**

质量分在 `_format_hit` 里从 `_source` 算一次挂到结果上（`quality_score` 字段），两处排序直接读，不重复计算：

- **RRF 阶段**（`retrieval.py`）：结果已按相关性主分排好、且全处于同一 RRF 量纲，用 `(-bucket, -quality)` 做一次**稳定**重排。
- **Rerank 阶段**（`reranking.py`）：窗口内是同量纲的 cross-encoder 分，排序键为 `(-bucket(rerank), -quality, pre_rank)`。

> **为什么不在 `search()` 里对最终列表全局重排？** 精排后头部窗口是 rerank 分（0~1）、尾部是 RRF 分（~0.02–0.05），量纲不同；全局重排会破坏窗口/尾部边界。所以质量键必须落在**每个分数可比的阶段内部**。

**④ 性能影响：可忽略**

质量分只在已返回、`_source` 已在内存的候选人上算（RRF 窗口 ≤ limit~100，精排 ≤ 20），纯字典遍历，无额外 ES 请求；学校名单复用 `_load_school_tiers()` 的进程内缓存。相比一次 kNN + cross-encoder API 调用，成本在噪声级别。

> **进阶（当前未启用）**：若数据量到十万级、或想让质量分参与召回排序，best practice 是在 indexing 阶段预计算 `quality_score` 写进主索引（`rank_feature` 或 `float` 字段），查询期零成本、还能用 `rank_feature` 查询做软加权；代价是权重调整需重建索引。当前 200 条规模下查询期计算更灵活，故先走查询期方案。
...
> **浏览模式的排序（无 q 或纯筛选）**：走 `match_all` / filter 浏览时没有相关性信号（`score` 全 0）。系统先让 ES 按 `application.apply_time` 降序拉取，再在 Python 侧按**候选人质量分降序稳定重排**（`_sort_browse_by_quality`）——用户是在"逛"候选人库，优质候选人应排在前；稳定排序使同质量分者保持投递时间序（越新越靠前）作为 tie-break。质量分复用 5.3.1 的 `compute_quality_score`，由 `_format_hit` 挂在结果上，此处直接读，不额外计算或请求 ES。
...
| **候选人质量二级排序** | 匹配分相同时一律回落 `resume_id` 字典序（与优劣无关） | 主分分桶后同桶内按质量分（学校/奖项/实习/项目/完整度）稳定重排 | 相关性决定"谁匹配"，质量决定"同分时谁更优"；质量只做 tie-break，永不覆盖相关性（详见 5.3.1） |
