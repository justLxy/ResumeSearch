...
| 模型框架       | sentence-transformers + PyTorch / requests        | 本地 embedding 推理、豆包 embedding API、Qwen3 rerank API |
...
| `ENABLE_RERANK`              | `True` | 是否对语义类查询启用 Qwen3 reranker          |
| `RERANK_TOP_N`               | 5      | RRF 后进入 reranker 的候选人数；开发阶段先只精排前 5 个以控制 API token |
