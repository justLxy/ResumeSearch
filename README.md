...
豆包 API 模式不会下载或加载本地模型，而是调用 OpenAI-compatible `/embeddings` 接口，并在本地继续做 L2 归一化，保证与 ES `cosine` 检索契约一致。
...
| NDCG@10      | 衡量前 10 条整体排序质量，越相关的候选排得越靠前分数越高；当前 qrels 是二值相关，所以相关候选越集中在前面，NDCG 越接近 1。 | 整体排序质量 |
...
- **看排序是否好**：优先看 `NDCG@10`、`MRR@10`、`P@5/P@10`。
...
评估脚本默认请求 100 条结果，同时输出整体指标和按查询类型分组的指标，便于观察 `entity`、`filter`、`skill_combo`、`natural_semantic`、`negative` 等不同查询族的收益与回退。当前 `eval_queries.jsonl` 使用静态 `relevant_ids`，与 `data/ai_generated.jsonl` 的 100 条模拟简历对齐，避免动态相关集为空时大量跳过用例。
...
当前评估集使用静态 `relevant_ids`，便于离线复现实验结果；如果要扩展评测集，建议仍优先手工维护稳定相关集。
